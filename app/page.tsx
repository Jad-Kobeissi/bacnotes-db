...
function Home() {
  const router = useRouter();
  return (
    <div className="text-center my-50 flex flex-col gap-20 px-2" id="home">
      <div>
        <div className="w-full flex items-center justify-center">
          <div className="flex items-center justify-center flex-col min-[400px]:max-w-3xl w-screen">
            <h1 className="text-[2.5rem] max-[600px]:text-[2.8rem] font-bold mb-6 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent max-[400px]:text-[2.5rem] leading-snug">
...
function Features() {
  function Card({
    ...
    return (
      <motion.div
        ...
        ref={ref}
        className={`shadow-sm hover:shadow-lg transition-shadow duration-300 rounded-lg p-6 flex flex-col gap-4 max-w-3xl mx-auto ${className}`}
      >
        {svg}
  ...
  return (
    <div className="mb-30 px-4 flex flex-col gap-6" id="features">
      <div className="flex gap-3 flex-col">
        <h1 className="text-[2rem] font-semibold text-center px-4">
          Everything You Need To Suceed
        </h1>
        <p className="text-(--secondary-text) text-center px-4 text-[1.2rem]">
          Powerful features designed to enhance your academic journey and foster
          collaboration
        </p>
      </div>
      <div className="grid grid-cols-[repeat(auto-fit,_minmax(220px,_1fr))] gap-4 place-items-center w-full">
...
function Footer() {
  return (
    <div className="h-[50vh] bg-linear-to-r from-blue-600 to-purple-600 text-background flex flex-col gap-2 items-center justify-center text-center">
      <h1 className="text-[1.8rem] font-semibold text-center">
        Ready to Join Your School's Community?
      </h1>
      <p>Start connecting, sharing, and achieving more with Bacconnect today</p>
      <button className="bg-background text-foreground px-5 py-1 rounded-md text-[1.2rem font-medium flex gap-2">
        Create Account{" "}
        <svg
          className="w-4"
          xmlns="http://www.w3.org/2000/svg"
